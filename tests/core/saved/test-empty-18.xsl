...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:my="http://my.example.com/my" xmlns:slax-func="http://exslt.org/functions" xmlns:slax-ext="http://xmlsoft.org/XSLT/namespace" version="1.0" extension-element-prefixes="slax-func slax-ext">
  ...
  <xsl:template match="foo[concat(../../id/text(), &quot;/&quot;, text())]">
    ...
    <xsl:value-of select="my:test(concat(local-game-prefix, opponent), answer)"/>
  ...
  <xsl:template match="/">
    <top>
      <xsl:variable xmlns:slax-ext="http://xmlsoft.org/XSLT/namespace" name="test" select="slax-ext:node-set(my:test(&quot;fish&quot;))"/>
      ...
      <xsl:variable name="that-temp-2">
        <your>was</your>
      </xsl:variable>
      <xsl:variable xmlns:slax-ext="http://xmlsoft.org/XSLT/namespace" name="that" select="slax-ext:node-set($that-temp-2)"/>
