...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:slax="http://code.google.com/p/libslax/slax" version="1.0" extension-element-prefixes="slax">
  ...
  <xsl:template match="/">
    <top>
      <slax:while xmlns:slax="http://code.google.com/p/libslax/slax" test="$count &gt; 0">
        <count>
          <xsl:value-of select="$count"/>
        </count>
        <slax:set-variable xmlns:slax="http://code.google.com/p/libslax/slax" name="count" svarname="slax-count" select="$count - 1"/>
      </slax:while>
      <boom/>
    </top>
  </xsl:template>
