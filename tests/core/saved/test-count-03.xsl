...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:slax="http://code.google.com/p/libslax/slax" version="1.0" extension-element-prefixes="slax">
  ...
  <xsl:template match="/">
    <out>
      ...
      <output>
        <xsl:value-of xmlns:slax="http://code.google.com/p/libslax/slax" select="slax:evaluate(&quot;$variable == ('th' _ 'is')&quot;)"/>
      </output>
      <test>
  ...
  <xsl:template match="*">
    <xsl:element name="{name()}">
      <xsl:value-of xmlns:slax="http://code.google.com/p/libslax/slax" select="slax:evaluate(concat(&quot;$sp_&quot;, name()))"/>
    </xsl:element>
  </xsl:template>
</xsl:stylesheet>
