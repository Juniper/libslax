...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:slax="http://code.google.com/p/libslax/slax" version="1.0" extension-element-prefixes="slax">
  ...
  <xsl:template match="author">
    ...
    <xsl:choose>
      <xsl:when test="$age &lt; 0">
        <slax:set-variable xmlns:slax="http://code.google.com/p/libslax/slax" name="problem" svarname="slax-problem" select="&quot;died before born&quot;"/>
      </xsl:when>
      <xsl:when test="$age &lt;= 40">
        <slax:set-variable xmlns:slax="http://code.google.com/p/libslax/slax" name="problem" svarname="slax-problem" select="&quot;died too young&quot;"/>
      </xsl:when>
    </xsl:choose>
    <xsl:if test="life-span/died - life-span/born &gt; 65">
      <slax:set-variable xmlns:slax="http://code.google.com/p/libslax/slax" name="problem" svarname="slax-problem" select="&quot;lived too long&quot;"/>
    </xsl:if>
    <xsl:if test="not($problem) and life-span/born &lt; 1900">
      <slax:set-variable xmlns:slax="http://code.google.com/p/libslax/slax" name="problem" svarname="slax-problem" select="&quot;ancient history&quot;"/>
      <slax:set-variable xmlns:slax="http://code.google.com/p/libslax/slax" name="tag" svarname="slax-tag" select="concat(&quot; (c. &quot;, life-span/born, &quot;)&quot;)"/>
    </xsl:if>
    <xsl:if test="$problem">
      <slax:append-to-variable xmlns:slax="http://code.google.com/p/libslax/slax" name="errors" svarname="slax-errors">
        <error>
          <name>
