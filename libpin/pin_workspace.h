...
typedef struct pin_workspace_s {
    ...
    pa_fixed_t *xw_nodes;	/* Pool of nodes (pin_node_t) */
    pa_istr_t *xw_names;	/* Array of names (element, attr, etc) */
    pa_pat_t *xw_names_index;	/* Patricia tree for names */
    pa_fixed_t *xw_ns_map; /* Map from prefixes to URLs (pin_ns_map_t) */
...
pin_namepool_string (pin_workspace_t *xwp, pa_atom_t name_atom)
{
...
static inline const char *
pin_textpool_string (pin_workspace_t *xwp, pa_atom_t atom)
{
    return pa_arb_atom_addr(xwp->xw_textpool, pa_arb_atom(atom));
}

static inline const char *
pin_get_attrib_string (pin_workspace_t *xwp, pin_node_t *nodep,
		      pa_atom_t name_atom)
{
    pa_atom_t atom = pin_get_attrib(xwp, nodep, name_atom);
    return (atom == PA_NULL_ATOM) ? NULL : pin_textpool_string(xwp, atom);
}
