...
/*
 * Field accessors for xmlParserCtxt and xmlSAXHandler, generated by
 * bin/gen-xmlaccessors.sh into a third file group (both structs live
 * in parser.h rather than tree.h or xpath.h). This include has to
 * come right after _xmlSAXHandler's closing brace, above -- the last
 * of the two parser-group structs to be defined in this file -- same
 * as the single include placement used for the four xpath-group
 * structs in xpath.h. Same LIBXML_ACCESSORS_NOINLINE flavor switch as
 * tree.h/xpath.h; by the time this header is reached, tree.h's
 * #include above has already resolved it.
 */
#ifdef LIBXML_ACCESSORS_NOINLINE
#include <libxml/gen/xmlaccessors-parser-decl.h>
#else
#include <libxml/gen/xmlaccessors-parser-inline.h>
#endif
