...
/*
 * Field accessors for xmlNodeSet and xmlXPathContext, generated by
 * bin/gen-xmlaccessors.sh into a second file group (both structs live
 * here in xpath.h rather than tree.h, which already includes its own
 * accessors before xpath.h -- and these structs -- exist). This
 * include has to come after xmlXPathContext's struct body, not right
 * after xmlNodeSet's earlier in this file, since the generated code
 * for both structs lives in the same file group. Same
 * ENABLE_ACCESSOR_INLINES flavor switch as tree.h; by the time this
 * header is reached, tree.h's #include above has already resolved
