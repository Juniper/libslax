...
/*
 * Field accessors for xmlNodeSet, xmlXPathContext, xmlXPathObject and
 * xmlXPathParserContext, generated by bin/gen-xmlaccessors.sh into a
 * second file group (all four structs live here in xpath.h rather
 * than tree.h, which already includes its own accessors before
 * xpath.h -- and these structs -- exist). This include has to come
 * after xmlXPathParserContext's struct body, the last of the four to
 * be defined in this file, since the generated code for all four
 * structs lives in the same file group. Same ENABLE_ACCESSOR_INLINES
 * flavor switch as tree.h; by the time this header is reached,
 * tree.h's #include above has already resolved
 * LIBXML_ACCESSORS_NOINLINE.
 */
#ifdef LIBXML_ACCESSORS_NOINLINE
#include <libxml/gen/xmlaccessors-xpath-decl.h>
#else
#include <libxml/gen/xmlaccessors-xpath-inline.h>
#endif
