...
pa_config_read_file (FILE *file)
{
	...
	xcvp = calloc(1, sizeof(*xcvp) + nlen + vlen);
	if (xcvp == NULL)
	    continue;
			  ...
			  int show_def, uint32_t def)
{
    ...
    if (show_def)
	slaxLog("config: looking for '%s' (default %u)", namebuf, def);
    else slaxLog("config: looking for '%s'", namebuf);

    for (xcvp = pa_config_vars; xcvp; xcvp = xcvp->xcv_next) {
	if (strcmp(xcvp->xcv_name, namebuf) != 0)
	    continue;

	slaxLog("config: found for '%s' -> '%s'",
