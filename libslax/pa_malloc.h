...
typedef struct pa_malloc_header_s {
    ...
    uint16_t pmh_size;		/* Size of in this allocation */
} pa_malloc_header_t;
...
#define PA_MALSM_ITEM_SHIFT	4 /* 1<<4 == 16, size of item */
#define PA_MALSM_ITEM_SIZE	(1 << PA_MALSM_ITEM_SIZE)
#define PA_MALSM_PAGE_SHIFT	12 /* 1<<12 == 4k items per page */
#define PA_MALSM_PAGE_MAX	(1 << PA_MALSM_SHIFT)
...
#define PA_MALSM_MAX_POW2 (PA_MALSM_ITEM_SHIFT + PA_MALSM_PAGE_SHIFT)

/*
 * Each page is divided into chunks, which are continquous items,
 * with a single item being the smallest chunk we can allocate.
...
typedef struct pa_malloc_info_s {
    ...
    pa_item_t pa_free[PA_MALSM_MAX_POW2]; /* The free list */
} pa_malloc_info_t;
