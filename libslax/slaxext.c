 ...
 */
static void
slaxExtHostname (xmlXPathParserContext *ctxt, int nargs)
{
    if (nargs == 0) {
	xmlXPathSetArityError(ctxt);
	return;
	...
	outstr = xmlStrdup(slax_empty_string);
 ...
 */
static void
slaxExtParseIp (xmlXPathParserContext *ctxt, int nargs)
{
    xsltTransformContextPtr tctxt;
    xmlDocPtr container;
    xmlXPathObjectPtr ret;
	...
	xsltGenericError(xsltGenericErrorContext,
			 "parse-ip error: Unknown address family\n");
	goto done;
    }

    /* Output: results[1] =>  Address */
    inet_ntop(af, &addr, address, sizeof(address));
    newp = slaxExtMakeTextNode(NULL, "parse-ip", address, strlen(address));
    if (newp) {
	xmlXPathNodeSetAdd(results, newp);
    ...
    newp = slaxExtMakeTextNode(NULL, "parse-ip", 
			  (af == AF_INET) ? "inet" : "inet6", 
			  (af == AF_INET) ? strlen("inet") : strlen("inet6"));
	...
	snprintf(address, sizeof(address), "%d", pfxlen);
	newp = slaxExtMakeTextNode(NULL, "parse-ip", address, strlen(address));
	...
	if (af == AF_INET) {
	    ...
	    newp = slaxExtMakeTextNode(NULL, "parse-ip", address, 
				      strlen(address));
...
slaxExtRegisterOther (const char *namespace)
{
    ...
    slaxRegisterFunction(namespace, "break_lines", slaxExtBreakLines);
